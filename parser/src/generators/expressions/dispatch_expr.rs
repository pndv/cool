...
pub(super) fn gen_partial_dispatch_expr(ident_token: Token, iter: &mut BufferedTokenIter) -> Result<Expression, String> {
  ...
  Ok(Expression::PartialDispatch { fn_name, param_list })
}

/// ...expr (seen before)... { `@` TYPE } `.` ID `(` { expr {{ `,` expr }} } 
pub(super) fn gen_partial_cast_dispatch(iter: &mut BufferedTokenIter) -> Result<Expression, String> {
  ...
  Ok(Expression::PartialCastDispatch { cast_type, fn_name, param_list })
}
...
fn gen_fn_param_list(iter: &mut BufferedTokenIter) -> Result<Vec<Expression>, String> {
  ...
  while fn_param_gen_iter.has_next() && !fn_param_gen_iter.peek_eq(&COMMA_TYPE) && fn_param_gen_iter.has_next() {
