...
fn gen_feature(iter: &mut BufferedTokenIter, read_till_token: &Token) -> Result<Feature, String> {
  ...
  let feature: Feature = match iter.peek() {
    Some(peeked_token) if *peeked_token == COLON_TYPE => gen_attribute_feature(ident_name, iter, read_till_token)?,

    Some(peeked_token) if *peeked_token == OPEN_PAREN_TYPE => gen_method_feature(ident_name, iter)?,
...
fn gen_method_feature(ident_name: Ident, iter: &mut BufferedTokenIter) -> Result<Feature, String> {
  ...
  Ok((ident_name, formals, method_return_type, Box::from(method_expr)).into())
}

fn gen_attribute_feature(ident_name: Ident, iter: &mut BufferedTokenIter, read_till_tokens: &Token) -> Result<Feature, String> {
  iter.consume_required(&COLON_TYPE)?;
...
mod test {
  ...
  fn test_feature_list() {
    let file = File::open("../test_resources/features/feature.list.cl_partial").expect("file not found");
    let mut token_iter: BufferedTokenIter = get_buffered_iter(file);
